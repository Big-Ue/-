...
CMonster::CMonster (int initHP, int initSP, int initMoney, int initExp, int initrough, string init_name, string init_eng_name) : CLifeEntity (initHP,initSP,initMoney,initExp,init_name), RoughDegree(initrough), eng_name(init_eng_name) {
	...
	cout << "one monster called " << init_name << " (" << init_eng_name << ") is created with <HP, SP, rough> = <" << initHP << ", " << initSP << ", " << initrough << "> 懸賞: " << initMoney << endl;
}
...
CMonster::CMonster (const CMonsterType *type) : CLifeEntity (1 + rand () % type->max_hp, 1 + rand () % type->max_sp, type->money, type->exp, type->name), RoughDegree(type->max_rough) {
	...
	cout << "Monster called " << type->name << " (" << eng_name << ") is created with <HP, SP, rough> = <" << this->getHP () << ", " << this->getSP () << ", " << this->getRough () << "> 懸賞: " << this->getMoney() << endl;
}
...
string CMonster::get_basic_data (){
	string output = this->getname () + string ("(") + this->eng_name + string (") <HP, SP, rough> = <") + to_string((long double)this->getHP ()) + string (", ")  + to_string((long double)this->getSP ()) + string (", ") + to_string((long double)this->getRough ()) + string("> 懸賞: ") + to_string((long double)this->getMoney());
	return output;	
}